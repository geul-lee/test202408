...
public class GetChanges_secondNearestMaturity {
	...
	static File sourceFolder = new File("c:/test/impliedDividend_secondNearestMaturity"), targetFile = new File("c:/test/changes_secondNearestMaturity.csv"), dayTFile, dayTMinusOneFile;	
	static File[] sourceFiles = sourceFolder.listFiles();
	static double daytimeChange, overnightChange, daytimeChange_underlying, overnightChange_underlying, rfr, rfrTMinusOne, rfrChange, lnMoneyness, lnMoneynessChange; 
	...
	private static void getChanges() {
		...
		Double tOpen = dayTImpliedDividendList.get(0).getImpliedDividend(),
			   tOpen_underlying = dayTImpliedDividendList.get(0).getUnderlying(),
			   tClose = dayTImpliedDividendList.get(dayTImpliedDividendList.size() - 1).getImpliedDividend(),
			   tClose_underlying = dayTImpliedDividendList.get(dayTImpliedDividendList.size() - 1).getUnderlying(),
			   tMinusOneClose = dayTMinusOneImpliedDividendList.get(dayTMinusOneImpliedDividendList.size() - 1).getImpliedDividend(),
		...
		overnightChange_underlying = (tOpen_underlying / tMinusOneClose_underlying) - 1;
