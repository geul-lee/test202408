...
public class GetImpliedDividend_koribor {
	...
	static File indexObservationFile, koriborObservationFile = new File("e:/koribor.csv"), sourceFolder = new File("e:/KOSPI200/1minute_sorted");
	...
	private static void estimateImpliedDividend(File file) {
		...
		for (int i = 0; i < indexObservationList.size(); i++) {
			...
			if (options[0] != null) {
				...
				rfr = getRfr(file.getName().substring(0, 10), tauInDays);
	...
	private static double getRfr(String dateString, double tau) {
		
		for (int i = 0; i < koriborObservationList.size(); i++) {
			
			KRKoriborObservation koriborObservation = koriborObservationList.get(i);
			if (koriborObservation.getDate().equals(dateString))
				return koriborObservation.getRate(tau);
	...
	private static String getTargetMaturity() {
		...
		if (ChronoUnit.DAYS.between(optionTransactionList.get(0).getTransactionDate(), maturityList.get(0)) >= 7)
			return maturityList.get(0).format(formatter);
		else
